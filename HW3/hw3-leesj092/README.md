[![Open in Visual Studio Code](https://classroom.github.com/assets/open-in-vscode-718a45dd9cf7e7f842a935f5ebbe5719a5e09af4491e668f4dbf3b35d5cca122.svg)](https://classroom.github.com/online_ide?assignment_repo_id=12175229&assignment_repo_type=AssignmentRepo)
...
![Annotated response #1 with screenshot capturing all changes](_figures/Response%201.png)

Annotations:
A. Improvement #1 (Space): I added negative space between the elements by specifying a height, width, and margin attributes.
B. Improvement #2 (Line): I added lines surrounding each element to allow users to more easily distinquish the different menus by adding a border attribute.
C. Improvement #3 (Texture): I changed HTML's default buttons to react- bootstrap's buttons in order to added more texture and contrast that align better with the overall color of the site.
D. Improvement #4 (Texture): I tried to highlight today's featured item by making to text bold, which adds a focal point and changes the texture of the page.
...
![Annotated response #2 with screenshot capturing changes](_figures/Response%202.jpg)

A. (Space & Balance) First, I would make the Container element of each menu a fixed size. This will help improve spacing and balance by providing a more symmetric view. Similarly, I would also organize the rows so that each row contains the same number of rows. For example, since we have a total of 6 menus I would organize each row to contain 3 rows each. This will also help with symmetry and balance for the page.

B. (Size & Value) I would also resize each container. It seems to me that each container at the moment is too large compared to the page title and other texts. For example, each menu text is as large as the name of the site. Resizing the elements will allow the users to view the site in one glance. I would also like to express the item name, description, and price in more various ways to better distinguish them. For now, they are all black texts which does not have much variety in value.

C. (Movement & Rhythm) For another improvement, I would also express the page title, bakery description, and today's feature different to add movment to the page. More specifically, I would like to guide the users to first view the Bakery's name, it's description and location, look at today's featured item, and finally add items to their cart. By adjusting the size and texture of these elements in order of importance, I can adjust the movement of the site as intended. One way I could do this is by resizing these elements in that order and also modifying the texture of the elements.
